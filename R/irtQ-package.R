...
#'
#' @description The \pkg{irtQ} package provides tools for fitting unidimensional
...
#' }
#'
#' @details The following five sections provide an overview of:
#' 1. The IRT models implemented in the \pkg{irtQ} package
#' 2. How to estimate item parameters for a linear test form
#' 3. How to perform pretest item calibration
#' using the fixed item parameter calibration (FIPC) method
#' 4. How to perform pretest item calibration using the fixed ability parameter
...
#' the [irtQ::est_irt()] function, which implements marginal maximum likelihood
...
#' - Estimate the latent ability distribution using a nonparametric histogram
#'   method (Woods, 2007)
#'
...
#' 1. Prepare the item metadata, including both the operational items (to be fixed)
...
#' true item parameters and true ability values using the [irtQ::simdat()] function.
#'
...
#' *Center for Advanced Studies in Measurement and Assessment Report, 49.*
#'
...
#' *Computer adaptive testing: A primer* (Chap. 4, pp.65-102). Hillsdale, NJ: Lawrence Erlbaum.
#'
